...
# Decorator to check if user is logged in or not, for protected endpoints
...
# Handles request for Login via facebook
...
def fbconnect():
    ...
    fb_qs = '?grant_type=fb_exchange_token&client_id'
    ...
    url = fb_token + fb_qs + '=%s&client_secret=%s&fb_exchange_token=%s' % (
    ...
    # Get user picture
    fb_pic = userinfo_url + '/picture?'
    url = fb_pic + 'access_token=%s&redirect=0&height=200&width=200' % token
    h = httplib2.Http()
    result = h.request(url, 'GET')[1]
    data = json.loads(result)
...
# Handles request for Logout, if login was done via facebook
...
# Handles login/signup request for google users
...
# Json API to view MenuItem related data
...
# JSON API to display all the restaurants
...
def showRestaurants():
    ...
    latestMenuItem = session.query(MenuItem).order_by(
        MenuItem.id.desc()).limit(5).all()
    if 'username' not in login_session:
        return render_template('publicrestaurant.html',
                               restaurants=restaurants,
                               latestMenuItems=latestMenuItem)
    else:
...
def newMenuItem(restaurant_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        return "<script>function myFunction() {alert('You are not authorized"\
               ...
               "own restaurant in order to add items.');}</script><body"\
               "onload='myFunction()'>"
    if request.method == 'POST':
